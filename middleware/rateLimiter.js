...
function make({ windowMs, max, message, keyFn }) {
  return rateLimit({
    ...
    keyGenerator: keyFn || ipKeyGenerator,
    handler(req, res) {
...
module.exports = {
  ...
  authedLimiter:  make({ windowMs: 60000,      max: 60,  message: 'Rate limit reached. Slow down.', keyFn: r => r.user?.id || ipKeyGenerator(r) }),
  contactLimiter: make({ windowMs: 300000,     max: 3,   message: 'Too many form submissions. Wait 5 minutes.' }),
  aiLimiter:      make({ windowMs: 60000,      max: 10,  message: 'AI chat limit reached. Wait 1 minute.', keyFn: r => r.user?.id || ipKeyGenerator(r) }),
